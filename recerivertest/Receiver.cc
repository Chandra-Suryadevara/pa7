...
Receiver::Receiver(const char* destinationHost, const char* destinationPort, const char* IPTYPE) {
    ...
    int rval = getaddrinfo(NULL, destinationPort, &hints, &results); // Use NULL for AI_PASSIVE
    ...
    freeaddrinfo(results); // Free the address info
...
void Receiver::receive_data() {
    ...
    if (activity == -1) {
        ...
        std::cerr << "No packets received within 15 seconds." << std::endl;
        return;
    }

    sockaddr_storage clientAddr;
    socklen_t clientAddrLen = sizeof(clientAddr);
    ssize_t receivedBytes = recvfrom(sock, buffer, sizeof(buffer), 0, (struct sockaddr*)&clientAddr, &clientAddrLen);
